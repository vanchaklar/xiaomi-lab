...
3. A cloud event control UI combines the cloud event listener with local START/STOP/DOCK controls and a live event feed. It keeps the proven local miIO DID unless the cloud device list has an exact DID match, and the UI remains usable even when all cloud event subscriptions are rejected.
