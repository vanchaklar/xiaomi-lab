...
3. Cloud capture confirmed the live map stream: `SIID 7 / EIID 1 map-points` arrives during cleaning with PIID 1 strings that decode cleanly into repeated `(x, y, type)` integer triplets. `SIID 7 / EIID 2 redraw-map` precedes/reset a new stream. The numeric type semantics are not yet established; observed values include 1, 2, 3, and 5. The cloud control UI now renders these triplets live while preserving the raw payloads.
